...
public class Statement {
	 ...
	 * @throws Exception
	 */
	public void execute(DataContextWithConnection ctx) throws Exception {
		...
		if (con != null) {
			...
			if (isQuery) {
				...
				if (firstRow) {
					...
					while (result.next()) {
						Object[] d = new Object[columns];
						for (int i = 0; i < keys.length; i++) {
							d[i] = result.getObject(i + 1);
						}
						daten.addElement(d);
					}
	...
	public Object queryObject(DataContextWithConnection ctx) throws Exception {
		DebugAssistent.doNullCheck(ctx);
