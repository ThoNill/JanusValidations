...
public class RhinoConnector {
	...
	private Vector<String> namen = null; // Die Namen der an das Skript zu
	...
	public void configure(DataDescription model) {
		...
		for (int i = 0; i < namen.size(); i++) {
			values[i] = model.getHandle(namen.elementAt(i));
		}
	}
			...
			if (s != null) {
				...
				for (int i = 0; i < values.length; i++) {
					Object jsOut = Context.javaToJS(ctx.getObject(values[i]),
							scope);
					ScriptableObject.putProperty(scope, namen.elementAt(i),
							jsOut);
				}
				// und ausf�hren
